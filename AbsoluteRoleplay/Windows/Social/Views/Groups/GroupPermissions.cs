...
namespace AbsoluteRP.Windows.Social.Views.Groups
{
    ...
    public static class GroupPermissions
    {
        ...
        public static bool HasPermission(Group group, Func<GroupRankPermissions, bool> permissionCheck, bool enableDetailedLogging = false)
        {
            ...
            {
                Plugin.PluginLog.Error($"[GroupPermissions] Error checking permission: {ex.Message}");
                if (enableDetailedLogging)
                    Plugin.PluginLog.Error($"[GroupPermissions] Stack trace: {ex.StackTrace}");
                return false;
            }
        }
        ...
        public static bool IsOwner(Group group)
        {
            ...
            {
                Plugin.PluginLog.Error($"[GroupPermissions] Error checking owner status: {ex.Message}");
                return false;
            }
        }
