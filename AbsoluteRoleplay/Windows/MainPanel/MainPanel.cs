    ...
    public void DrawMainUI()
    {
        try
        {
              ...
              !string.IsNullOrEmpty(Plugin.plugin.Configuration.account.accountName))
            {
                ...
                // Build the list of names for the combo: "None" + saved names
                var fauxNames = Plugin.plugin.Configuration.savedFauxNames ?? new List<string>();
                var comboItems = new List<string> { "None (Use Real Name)" };
                comboItems.AddRange(fauxNames);

                // Sync selectedFauxNameIndex with config on each frame
                string currentFauxName = string.Empty;
                if (Plugin.PlayerState.ContentId != 0 &&
                    Plugin.plugin.Configuration.IdentifyAs.TryGetValue(Plugin.PlayerState.ContentId, out var identifyAs))
                {
                    currentFauxName = identifyAs.Item1;
                }
                else if (!string.IsNullOrEmpty(Plugin.plugin.Configuration.fauxName))
                {
                    // Fallback to fauxName config when ContentId is not available
                    currentFauxName = Plugin.plugin.Configuration.fauxName;
                }

                // Sync the index from config
                if (!string.IsNullOrEmpty(currentFauxName))
                {
                    int foundIndex = fauxNames.IndexOf(currentFauxName);
                    if (foundIndex >= 0)
                        selectedFauxNameIndex = foundIndex + 1; // +1 because "None" is at index 0
                    else
                        selectedFauxNameIndex = 0;
                }
                else
                {
                    selectedFauxNameIndex = 0;
                ...
                // Combo dropdown
                float comboWidth = buttonWidth * 1.7f;
                ImGui.SetNextItemWidth(comboWidth);
                if (ImGui.Combo("##FauxNameCombo", ref selectedFauxNameIndex, comboItems.ToArray(), comboItems.Count))
                {
                    if (selectedFauxNameIndex == 0)
                    {
                        // Clear faux name - revert to real character name
                        if (Plugin.PlayerState.ContentId != 0)
                        {
                            Plugin.plugin.Configuration.IdentifyAs.Remove(Plugin.PlayerState.ContentId);
                        }
                        Plugin.plugin.Configuration.fauxName = string.Empty;
                        Plugin.plugin.Configuration.Save();

                        // Broadcast real name to other players
                        if (Plugin.PlayerState.ContentId != 0)
                        {
                            string realName = Plugin.ClientState.LocalPlayer?.Name.ToString() ?? Plugin.character.characterName;
                            DataSender.SendFauxNameBroadcast(Plugin.character, realName, true, Plugin.VisiblePlayers());
                            DataSender.SendFauxNameBroadcast(Plugin.character, realName, true, new List<Dalamud.Game.ClientState.Objects.SubKinds.IPlayerCharacter> { Plugin.ClientState.LocalPlayer });
                        }
                    }
                    else
                    {
                        // Set faux name
                        string selectedName = fauxNames[selectedFauxNameIndex - 1];
                        if (Plugin.PlayerState.ContentId != 0)
                        {
                            Plugin.plugin.Configuration.IdentifyAs[Plugin.PlayerState.ContentId] = (selectedName, 0);
                        }
                        Plugin.plugin.Configuration.fauxName = selectedName;
                        Plugin.plugin.Configuration.Save();

                        // Broadcast to other players
                        if (Plugin.PlayerState.ContentId != 0)
                        {
                            DataSender.SendFauxNameBroadcast(Plugin.character, selectedName, true, Plugin.VisiblePlayers());
                            DataSender.SendFauxNameBroadcast(Plugin.character, selectedName, true, new List<Dalamud.Game.ClientState.Objects.SubKinds.IPlayerCharacter> { Plugin.ClientState.LocalPlayer });
                        }
                    }
                }
                ...
                // Add button
                if (ImGui.Button("+##AddFauxName", new Vector2(buttonHeight / 2f, buttonHeight / 2f)))
                {
                    openAddFauxNamePopup = true;
                    newFauxNameInput = string.Empty;
                    ImGui.OpenPopup("Add Display Name");
                }
                if (ImGui.IsItemHovered())
                {
                    ImGui.SetTooltip("Add a new display name");
                }

                ImGui.SameLine();

                // Remove button (only if a name is selected)
                using (ImRaii.Disabled(selectedFauxNameIndex == 0 || fauxNames.Count == 0))
                {
                    if (ImGui.Button("-##RemoveFauxName", new Vector2(buttonHeight / 2f, buttonHeight / 2f)))
                    {
                        if (selectedFauxNameIndex > 0 && selectedFauxNameIndex <= fauxNames.Count)
                        {
                            string nameToRemove = fauxNames[selectedFauxNameIndex - 1];
                            fauxNames.RemoveAt(selectedFauxNameIndex - 1);
                            Plugin.plugin.Configuration.savedFauxNames = fauxNames;

                            // If this was the active name, clear it
                            if (currentFauxName == nameToRemove && Plugin.PlayerState.ContentId != 0)
                            {
                                Plugin.plugin.Configuration.IdentifyAs.Remove(Plugin.PlayerState.ContentId);
                                Plugin.plugin.Configuration.fauxName = string.Empty;
                                DataSender.SendFauxNameBroadcast(Plugin.character, string.Empty, false, Plugin.VisiblePlayers());
                            }
                ...
                if (ImGui.IsItemHovered(ImGuiHoveredFlags.AllowWhenDisabled))
                {
                    ImGui.SetTooltip("Remove selected display name");
                }

                // Add Faux Name Popup
                if (openAddFauxNamePopup)
                {
                    ImGui.SetNextWindowSize(new Vector2(350, 0), ImGuiCond.Always);
                    if (ImGui.BeginPopupModal("Add Display Name", ref openAddFauxNamePopup, ImGuiWindowFlags.AlwaysAutoResize))
                    {
                        ImGui.Text("Enter a display name:");
                        ImGui.SetNextItemWidth(300);
                        ImGui.InputText("##NewFauxNameInput", ref newFauxNameInput, 64);

                        bool nameExists = fauxNames.Contains(newFauxNameInput, StringComparer.OrdinalIgnoreCase);
                        bool nameEmpty = string.IsNullOrWhiteSpace(newFauxNameInput);

                        if (nameExists)
                        {
                            ImGui.TextColored(new Vector4(1, 0.5f, 0, 1), "This name already exists.");
                        }

                        using (ImRaii.Disabled(nameEmpty || nameExists))
                        {
                            if (ImGui.Button("Add", new Vector2(120, 0)))
                            {
                                fauxNames.Add(newFauxNameInput.Trim());
                                Plugin.plugin.Configuration.savedFauxNames = fauxNames;
                                Plugin.plugin.Configuration.Save();
                                openAddFauxNamePopup = false;
