...
namespace AbsoluteRoleplay.Helpers
{
    internal class ItemGrid
    {
        ...
        private static Dictionary<int, Defines.ItemDefinition> DraggedSlotContents = null;

        public static void DrawGrid(Plugin plugin, Dictionary<int, Defines.ItemDefinition> slotContents, bool isTrade)
        {
                ...
                for (int x = 0; x < GridSize; x++)
                {
                    ...
                    if (ImGui.BeginPopup($"##contextMenu{slotIndex}"))
                    {
                        if (ImGui.MenuItem("Delete"))
                        {
                            ...
                            List<Defines.ItemDefinition> newItemList = new List<Defines.ItemDefinition>();
                            for (int i = 0; i < slotContents.Count; i++)
                            {
                                if (slotContents.ContainsKey(i) && slotContents[i].name != string.Empty)
                                {
                                    newItemList.Add(new Defines.ItemDefinition
                                    {
                            ...
                            DataSender.SendItemOrder(ProfileWindow.currentProfileIndex, newItemList);
                        }
                        if (ImGui.MenuItem("Duplicate"))
                        {
                            DataSender.SendItemCreation(ProfileWindow.currentProfileIndex, slotContents[slotIndex].name, slotContents[slotIndex].description, slotContents[slotIndex].type, slotContents[slotIndex].subtype, (uint)slotContents[slotIndex].iconID, slotContents[slotIndex].quality);
                        }
